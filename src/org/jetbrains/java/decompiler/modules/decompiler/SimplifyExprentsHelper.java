    ...
    if (stat.type == Statement.TYPE_IF && stat.getExprents() == null) {
      ...
      if (statement.iftype == IfStatement.IFTYPE_IFELSE) {
        ...
        if (ifStatement.getExprents() != null && ifStatement.getExprents().size() == 1 &&
          ...
          // Not standard stack assign ternary, we need to process differently

          // Neither statement should have exprents, both statements should have 1 successor, and both successors must point to the same place
          // Bad things seem to happen when running this too early, so we make sure that the if statements don't have any experents in the head (which would happen before stack simplification)
          if (ifStatement.type == Statement.TYPE_IF && elseStatement.type == Statement.TYPE_IF &&
            ...
            statement.getParent().getStats().removeWithKey(destination.id);
            ...
            // Update closure
            if (destEdge.closure == stat.getParent()) {
              destEdge.closure = stat;
            }

            // Add control flow between the if statement and the destination's next statement
            statement.addSuccessor(new StatEdge(StatEdge.TYPE_REGULAR, statement, destEdge.getDestination(), statement.getParent()));
